...
func newInstallSkillCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "install-skill",
		Short: "Install the AetherSearch CLI agent skill file",
		Long: `Install the bundled SKILL.md so that AI coding agents can discover and use
the AetherSearch CLI as a tool.

Files are written to the canonical .agents/skills/aethersearch-cli/ directory. For
...
Use --agent to target specific agents (can be repeated).`,
		Example: `  aethersearch-cli install-skill
  aethersearch-cli install-skill --global
  aethersearch-cli install-skill --agent claude-code
  aethersearch-cli install-skill --copy`,
