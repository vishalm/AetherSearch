...
func (m authModel) View() string {
	...
	b.WriteString("\n")
	b.WriteString("  \x1b[1;35mAetherSearch CLI\x1b[0m\n")
...
func newServeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "serve",
		Short: "Serve the AetherSearch TUI over SSH",
		Long: `Start an SSH server that presents the interactive AetherSearch chat TUI to
...
environment variable (the --host-key flag takes precedence).`,
  ...
  aethersearch-cli serve --idle-timeout 30m --max-session-timeout 2h`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			log.Info("Starting AetherSearch SSH server", "addr", addr)
