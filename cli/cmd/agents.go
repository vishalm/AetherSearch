...
func newAgentsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `List all visible agents configured on the AetherSearch server.

By default, output is a human-readable table with ID, name, and description.
Use --json for machine-readable output.`,
		Example: `  aethersearch-cli agents
  aethersearch-cli agents --json
  aethersearch-cli agents --json | jq '.[].name'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("failed to list agents: %w\n  Check your connection with: aethersearch-cli validate-config", err)
			}
