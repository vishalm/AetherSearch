...
func newAskCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Send a one-shot question to an AetherSearch agent and print the response.
...
to a temp file. Set --max-output 0 to disable truncation.`,
		...
		Example: `  aethersearch-cli ask "What connectors are available?"
  aethersearch-cli ask --agent-id 3 "Summarize our Q4 revenue"
  aethersearch-cli ask --json "List all users" | jq '.event.content'
  cat error.log | aethersearch-cli ask --prompt "Find the root cause"
  echo "what is aethersearch?" | aethersearch-cli ask`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := config.Load()
			if !cfg.IsConfigured() {
				return exitcodes.New(exitcodes.NotConfigured, "AetherSearch CLI is not configured\n  Run: aethersearch-cli configure")
			}
...
func resolveQuestion(args []string, prompt string) (string, error) {
	...
	default:
		return "", exitcodes.New(exitcodes.BadRequest, "no question provided\n  Usage: aethersearch-cli ask \"your question\"\n  Or:    echo \"context\" | aethersearch-cli ask --prompt \"your question\"")
	}
}
