...
```shell
uv pip install aethersearch-cli
```
...
This prompts for your AetherSearch server URL and API key, tests the connection, and saves config to `~/.config/aethersearch-cli/config.json`.
...
aethersearch-cli ask --agent-id 5 "Summarize this topic"
aethersearch-cli ask --json "Hello"
```
...
aethersearch-cli serve --host 0.0.0.0 --port 2222
...
The CLI is distributed as a Python package via [PyPI](https://pypi.org/project/aethersearch-cli/). The build system uses [hatchling](https://hatch.pypa.io/) with [manygo](https://github.com/nicholasgasior/manygo) to cross-compile Go binaries into platform-specific wheels.
