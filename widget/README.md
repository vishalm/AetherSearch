...
```html
<!-- Load the widget -->
<script type="module" src="https://cdn.aethersearch.app/widget/1.0/dist/aethersearch-widget.js"></script>

<!-- Configure and display -->
<aethersearch-chat-widget
  backend-url="https://cloud.aethersearch.app/api"
  ...
  └─ Shadow DOM
      ...
      └─ Chat Container
          ...
          └─ Input Area
              ├─ Text Input
              ├─ Send Button
              └─ "Powered by AetherSearch" Footer
```
...
| ------------- | ------ | -------------------------------------------------------------------- |
| `backend-url` | string | Your AetherSearch backend API URL (or set `VITE_WIDGET_BACKEND_URL` in .env) |
...
| `logo`             | string | AetherSearch logo     | URL to custom logo image                 |
...
```html
<aethersearch-chat-widget backend-url="https://cloud.aethersearch.app/api" api-key="on_abc123">
</aethersearch-chat-widget>
```
...
```html
<aethersearch-chat-widget mode="launcher"></aethersearch-chat-widget>
```
...
<div class="chat-container">
  <aethersearch-chat-widget mode="inline"></aethersearch-chat-widget>
</div>
```
...
- Node.js 18+ and npm
- Access to AetherSearch backend API
...
│   └── assets/
│       └── logo.ts              # Default AetherSearch logo (base64)
├── dist/                        # Build output
├── index.html
├── package.json
├── vite.config.ts
...
The widget communicates with these AetherSearch backend endpoints:
...
3. **Deploy `dist/aethersearch-widget.js` to your server**

4. **Customer embed:**
   ```html
   <script type="module" src="https://your-cdn.com/aethersearch-widget.js"></script>
   <aethersearch-chat-widget
     agent-id="1"
     agent-name="Support"
...
```css
--theme-primary-05: #1c1c1c; /* Buttons, accents (aethersearch-ink-95) */
--theme-primary-06: #000000; /* Hover state (aethersearch-ink-100) */
