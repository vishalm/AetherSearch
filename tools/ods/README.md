...
Launch AetherSearch docker containers using docker compose.
...
View logs from running AetherSearch docker containers. Service names are available as
...
Pull the latest images for AetherSearch docker containers.
...
Manage the AetherSearch devcontainer. Also available as `ods dc`.
...
The devcontainer image is published to `aethersearch/aethersearch-devcontainer` and
...
with the pip version of `ods` installed in the AetherSearch venv._
