// Custom title bar for AetherSearch Desktop
// This script injects a draggable title bar that matches AetherSearch design system

(function () {
  const TITLEBAR_ID = "aethersearch-desktop-titlebar";
  const TITLEBAR_HEIGHT = 36;
  const STYLE_ID = "aethersearch-desktop-titlebar-style";
  const VIEWPORT_VAR = "--aethersearch-desktop-viewport-height";
  ...
  function injectStyles() {
    ...
    style.textContent = `
      :root {
        --aethersearch-desktop-titlebar-height: ${TITLEBAR_HEIGHT}px;
        --aethersearch-desktop-viewport-height: 100dvh;
        --aethersearch-desktop-safe-height: calc(var(--aethersearch-desktop-viewport-height) - var(--aethersearch-desktop-titlebar-height));
      }

      @supports not (height: 100dvh) {
        :root {
          --aethersearch-desktop-viewport-height: 100vh;
        }
      }

      html,
      body {
        height: var(--aethersearch-desktop-viewport-height);
        min-height: var(--aethersearch-desktop-viewport-height);
        margin: 0;
        padding: 0;
        overflow: hidden;
      }
      ...
      body {
        padding-top: var(--aethersearch-desktop-titlebar-height) !important;
      ...
      body > div#root,
      body > main {
        height: var(--aethersearch-desktop-safe-height);
        min-height: var(--aethersearch-desktop-safe-height);
        overflow: auto;
      }

      /* Override common Tailwind viewport helpers so content fits under the titlebar */
      .h-screen {
        height: var(--aethersearch-desktop-safe-height) !important;
      }

      .min-h-screen {
        min-height: var(--aethersearch-desktop-safe-height) !important;
      }

      .max-h-screen {
        max-height: var(--aethersearch-desktop-safe-height) !important;
      }
      ...
      #${TITLEBAR_ID} {
        ...
        height: var(--aethersearch-desktop-titlebar-height);
      }
  ...
  function buildTitleBar() {
    ...
    // Apply styles matching AetherSearch design system with translucent glass effect
