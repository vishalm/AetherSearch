...
const DEFAULT_SERVER_URL: &str = "https://cloud.aethersearch.app";
const CONFIG_FILE_NAME: &str = "config.json";
#[cfg(target_os = "macos")]
const TITLEBAR_SCRIPT: &str = include_str!("../../src/titlebar.js");
const TRAY_ID: &str = "aethersearch-tray";
...
(() => {
  if (window.__AETHERSEARCH_CHAT_LINK_INTERCEPT_INSTALLED__) {
    return;
  }

  window.__AETHERSEARCH_CHAT_LINK_INTERCEPT_INSTALLED__ = true;
...
(() => {
  if (window.__AETHERSEARCH_MENU_KEY_HANDLER__) return;
  window.__AETHERSEARCH_MENU_KEY_HANDLER__ = true;
...
(() => {
  if (window.__AETHERSEARCH_CONSOLE_CAPTURE__) return;
  window.__AETHERSEARCH_CONSOLE_CAPTURE__ = true;
...
fn get_config_dir() -> Option<PathBuf> {
    ProjectDirs::from("app", "aethersearch", "aethersearch-desktop").map(|dirs| dirs.config_dir().to_path_buf())
}
...
fn is_debug_mode() -> bool {
    std::env::args().any(|arg| arg == "--debug") || std::env::var("AETHERSEARCH_DEBUG").is_ok()
}
...
fn trigger_new_window(app: &AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let window_label = format!("aethersearch-{}", uuid::Uuid::new_v4());
...
fn open_docs() {
    let _ = open_in_default_browser("https://docs.aethersearch.app");
}
...
fn handle_open_debug_log() {
    ...
    if !log_path.exists() {
        eprintln!("[AETHERSEARCH DEBUG] Log file does not exist yet: {:?}", log_path);
        return;
    }
...
fn setup_app_menu(app: &AppHandle) -> tauri::Result<()> {
    ...
    let docs_item = MenuItem::with_id(app, "open_docs", "AetherSearch Documentation", true, None::<&str>)?;
...
fn build_tray_menu(app: &AppHandle) -> tauri::Result<Menu<Wry>> {
    let open_app = MenuItem::with_id(app, TRAY_MENU_OPEN_APP_ID, "Open AetherSearch", true, None::<&str>)?;
    ...
    let quit = PredefinedMenuItem::quit(app, Some("Quit AetherSearch"))?;
...
fn setup_tray_icon(app: &AppHandle) -> tauri::Result<()> {
    let mut builder = TrayIconBuilder::with_id(TRAY_ID).tooltip("AetherSearch");
...
fn main() {
    ...
    let debug_log_file = if debug_mode {
        eprintln!("[AETHERSEARCH DEBUG] Debug mode enabled");
        if let Some(path) = get_debug_log_path() {
            eprintln!("[AETHERSEARCH DEBUG] Frontend logs: {}", path.display());
        }
        eprintln!("[AETHERSEARCH DEBUG] DevTools will open automatically");
        eprintln!("[AETHERSEARCH DEBUG] Capturing console.log/warn/error/info/debug from webview");
        init_debug_log_file()
    } else {
        None
    };
