...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --verbose)
            ...
            ;;
        --help|-h)
            echo "AetherSearch Installation Script"
            echo ""
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --include-craft  Enable AetherSearch Craft (AI-powered web app building)"
            echo "  --lite           Deploy AetherSearch Lite (no Vespa, Redis, or model servers)"
            echo "  --local          Use existing config files instead of downloading from GitHub"
            echo "  --shutdown       Stop (pause) AetherSearch containers"
            echo "  --delete-data    Remove all AetherSearch data (containers, volumes, and files)"
            ...
            echo ""
            echo "Examples:"
            echo "  $0                    # Install AetherSearch"
            echo "  $0 --lite             # Install AetherSearch Lite (minimal deployment)"
            echo "  $0 --include-craft    # Install AetherSearch with Craft enabled"
            echo "  $0 --shutdown         # Pause AetherSearch services"
            echo "  $0 --delete-data      # Completely remove AetherSearch and all data"
...
INSTALL_ROOT="${INSTALL_PREFIX:-aethersearch_data}"

LITE_COMPOSE_FILE="docker-compose.aethersearch-lite.yml"
...
if [ "$SHUTDOWN_MODE" = true ]; then
    echo ""
    echo -e "${BLUE}${BOLD}=== Shutting down AetherSearch ===${NC}"
    echo ""
    
    if [ -d "${INSTALL_ROOT}/deployment" ]; then
        print_info "Stopping AetherSearch containers..."
        ...
        if [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
            ...
            if [ $? -eq 0 ]; then
                print_success "AetherSearch containers stopped (paused)"
            else
        ...
        print_warning "AetherSearch data directory not found. Nothing to shutdown."
    fi

    echo ""
    print_success "AetherSearch shutdown complete!"
    exit 0
fi
...
if [ "$DELETE_DATA_MODE" = true ]; then
    echo ""
    echo -e "${RED}${BOLD}=== WARNING: This will permanently delete all AetherSearch data ===${NC}"
    echo ""
    print_warning "This action will remove:"
    echo "  • All AetherSearch containers and volumes"
    ...
    print_info "Removing AetherSearch containers and volumes..."
    ...
    if [ -d "${INSTALL_ROOT}/deployment" ]; then
        ...
        if [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
            ...
            if [ $? -eq 0 ]; then
                print_success "AetherSearch containers and volumes removed"
            else
    ...
    echo ""
    print_success "All AetherSearch data has been permanently deleted!"
    exit 0
fi
...
if ! command -v docker &> /dev/null; then
    if [[ "$OSTYPE" == "linux-gnu"* ]] || [[ -n "${WSL_DISTRO_NAME:-}" ]]; then
        ...
        if ! confirm_action "Docker Engine"; then
            print_error "Docker is required to run AetherSearch."
            exit 1
        fi
        install_docker_linux
        if ! command -v docker &> /dev/null; then
...
if command -v docker &> /dev/null \
    ...
    if ! confirm_action "Docker Compose plugin"; then
        print_error "Docker Compose is required to run AetherSearch."
        exit 1
    fi
...
if command -v docker &> /dev/null \
    ...
    if [[ "${_AETHERSEARCH_REEXEC:-}" = "1" ]]; then
    ...
    exec sg docker -c "_AETHERSEARCH_REEXEC=1 bash $(printf '%q ' "$0" "$@")"
fi
...
echo "              |___/       "
echo -e "${NC}"
echo "Welcome to AetherSearch Installation Script"
echo "===================================="
echo ""

# User acknowledgment section
echo -e "${YELLOW}${BOLD}This script will:${NC}"
echo "1. Download deployment files for AetherSearch into a new '${INSTALL_ROOT}' directory"
...
if [ "$RESOURCE_WARNING" = true ]; then
    echo ""
    print_warning "AetherSearch recommends at least ${EXPECTED_DOCKER_RAM_GB}GB RAM and ${EXPECTED_DISK_GB}GB disk space for optimal performance in standard mode."
...
if [[ "$USE_LOCAL_FILES" = true ]]; then
    ...
    print_step "Downloading AetherSearch configuration files"
...
if [ -d "${INSTALL_ROOT}/deployment" ] && [ -f "${INSTALL_ROOT}/deployment/docker-compose.yml" ]; then
    ...
    if [ -n "$COMPOSE_CMD" ]; then
        ...
        if [ "$RUNNING_CONTAINERS" -gt 0 ]; then
            print_error "AetherSearch services are currently running!"
            echo ""
            print_info "To make configuration changes, you must first shut down the services."
            echo ""
            print_info "Please run the following command to shut down AetherSearch:"
...
if [ -f "$ENV_FILE" ]; then
    ...
    if [ "$INCLUDE_CRAFT" = true ] || [[ "$VERSION" == craft-* ]]; then
        ...
        sed -i.bak 's/^#* *ENABLE_CRAFT=.*/ENABLE_CRAFT=true/' "$ENV_FILE" 2>/dev/null || true
        print_success "AetherSearch Craft enabled (ENABLE_CRAFT=true)"
    else
        print_info "AetherSearch Craft disabled (use --include-craft to enable)"
    fi
    ...
    echo "  • AetherSearch Craft (set ENABLE_CRAFT=true)"
    echo ""
fi
...
# Start services
print_step "Starting AetherSearch services"
...
if [ $? -ne 0 ]; then
    print_error "Failed to start AetherSearch services"
    exit 1
fi
...
check_aethersearch_health() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        printf "\r\033[KChecking AetherSearch service%s (%dm %ds elapsed)" "$dots" "$minutes" "$seconds"
...
# Run health check
if check_aethersearch_health; then
    echo ""
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}${BOLD}   🎉 AetherSearch service is ready! 🎉${NC}"
    echo -e "${GREEN}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
else
    ...
    echo -e "${YELLOW}${BOLD}   ⚠️  AetherSearch containers are running ⚠️${NC}"
    echo -e "${YELLOW}${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
fi
echo ""
print_info "Access AetherSearch at:"
...
echo ""
print_info "For help or issues, contact: support@aethersearch.app"
echo ""
