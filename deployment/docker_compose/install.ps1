...
function Print-AetherSearchError{ param([string]$Message) Write-Host "[X]  $Message" -ForegroundColor Red }
...
function Prompt-DeploymentMode {
    ...
    if ($modeChoice -eq "2") {
        ...
        if (-not (Ensure-AetherSearchFile $LiteOverlayPath "$($script:GitHubRawUrl)/$($script:LiteComposeFile)" $script:LiteComposeFile)) { exit 1 }
    }
}
...
function Assert-NotCraftLite {
    ...
    Print-AetherSearchError "Cannot use a craft image tag ($Tag) with Lite mode."
...
function Download-AetherSearchFile {
...
function Ensure-AetherSearchFile {
    param([string]$Path, [string]$Url, [string]$Description)
    if ($Local) {
        if (Test-Path $Path) { Print-Success "Using existing $Description"; return $true }
        Print-AetherSearchError "Required file missing: $Description ($Path)"
        return $false
    }
    Print-Info "Downloading $Description..."
    try {
        Download-AetherSearchFile -Url $Url -Output $Path
        Print-Success "$Description downloaded"
        return $true
    } catch {
        Print-AetherSearchError "Failed to download $Description"
        return $false
    }
}
...
function Test-AetherSearchHealth {
    param([int]$Port)
    Print-Info "Checking AetherSearch service health..."
    ...
    for ($attempt = 1; $attempt -le 600; $attempt++) {
        ...
        Write-Host -NoNewline "`rChecking AetherSearch service${dots}${pad} (${m}m ${s}s elapsed)"
...
function Show-AetherSearchHelp {
    $help = "AetherSearch Installation Script for Windows`n"
    $help += "`nUsage: .\install.ps1 [OPTIONS]`n"
    $help += "`nOptions:"
    $help += "`n  -IncludeCraft  Enable AetherSearch Craft (AI-powered web app building)"
    $help += "`n  -Lite          Deploy AetherSearch Lite (no Vespa, Redis, or model servers)"
    $help += "`n  -Local         Use existing config files instead of downloading from GitHub"
    $help += "`n  -Shutdown      Stop (pause) AetherSearch containers"
    $help += "`n  -DeleteData    Remove all AetherSearch data (containers, volumes, and files)"
    ...
    $help += "`n`nExamples:"
    $help += "`n  .\install.ps1                    # Install AetherSearch"
    $help += "`n  .\install.ps1 -Lite              # Install AetherSearch Lite"
    $help += "`n  .\install.ps1 -IncludeCraft      # Install with Craft enabled"
    $help += "`n  .\install.ps1 -Shutdown          # Pause AetherSearch services"
    $help += "`n  .\install.ps1 -DeleteData        # Completely remove AetherSearch"
    $help += "`n  .\install.ps1 -Local             # Re-run using existing config"
    $help += "`n  .\install.ps1 -NoPrompt          # Non-interactive install"
    Write-Host $help
}

function Invoke-AetherSearchShutdown {
    Write-Host "`n=== Shutting down AetherSearch ===`n" -ForegroundColor Cyan
    $deployDir = Join-Path $script:InstallRoot "deployment"
    if (-not (Test-Path (Join-Path $deployDir "docker-compose.yml"))) {
        Print-Warning "AetherSearch deployment not found. Nothing to shutdown."
        return
    }
    if (-not (Initialize-ComposeCommand)) { Print-AetherSearchError "Docker Compose not found."; exit 1 }
    $stopArgs = @("stop")
    $result = Invoke-Compose -AutoDetect @stopArgs
    if ($result -ne 0) { Print-AetherSearchError "Failed to stop containers"; exit 1 }
    Print-Success "AetherSearch containers stopped (paused)"
}

function Invoke-AetherSearchDeleteData {
    Write-Host "`n=== WARNING: This will permanently delete all AetherSearch data ===`n" -ForegroundColor Red
    Print-Warning "This action will remove all AetherSearch containers, volumes, files, and user data."
    if (Test-Interactive) {
        $confirm = Prompt-OrDefault "Type 'DELETE' to confirm" ""
        if ($confirm -ne "DELETE") { Print-Info "Operation cancelled."; return }
    } else {
        Print-AetherSearchError "Cannot confirm destructive operation in non-interactive mode."
        exit 1
    }
    $deployDir = Join-Path $script:InstallRoot "deployment"
    if ((Test-Path (Join-Path $deployDir "docker-compose.yml")) -and (Initialize-ComposeCommand)) {
        ...
        else { Print-AetherSearchError "Failed to remove containers" }
    }
    if (Test-Path $script:InstallRoot) {
        Remove-Item -Recurse -Force $script:InstallRoot
        Print-Success "Data directories removed"
    }
    Print-Success "All AetherSearch data has been permanently deleted!"
}
...
function Wait-ForDockerDaemon {
    ...
    while ($waited -lt $MaxWait) {
        ...
        if ($currentError) {
            ...
            if ($unchangedErrorCount -ge 5) {
                Write-Host ""; Print-AetherSearchError "Docker daemon is not starting. Persistent error after ${waited}s:"
    ...
    Write-Host ""; Print-AetherSearchError "Docker daemon did not respond within ${MaxWait} seconds."
...
function Register-DockerService {
    ...
    if (-not $dockerdPath) {
        Print-AetherSearchError "Could not find dockerd.exe to register as a service."
        return $false
    }
    ...
    if (-not (Get-Service docker -ErrorAction SilentlyContinue)) {
        Print-AetherSearchError "Failed to register Docker as a Windows service."
        return $false
    }
...
function Install-DockerEngine {
    ...
    if (-not $installed) {
        ...
        try {
            ...
            Download-AetherSearchFile -Url "https://download.docker.com/win/static/stable/x86_64/$($latestZip.href)" -Output $zipPath
    ...
    if (-not $installed) {
        Print-AetherSearchError "Could not install Docker Engine on Windows Server."
        Print-Info "Install manually: https://docs.docker.com/engine/install/binaries/#install-server-and-client-binaries-on-windows"
        exit 1
    }

    try { Start-Service docker -ErrorAction Stop; Print-Success "Docker service started" }
    catch { Print-AetherSearchError "Failed to start Docker service: $_"; exit 1 }
    Install-ComposePlugin
    if (-not (Wait-ForDockerDaemon -MaxWait 30)) { Print-AetherSearchError "Docker installed but daemon not responding."; exit 1 }
...
function Install-ComposePlugin {
    ...
    try {
        Download-AetherSearchFile -Url "https://github.com/docker/compose/releases/latest/download/docker-compose-windows-x86_64.exe" -Output (Join-Path $dest "docker-compose.exe")
...
function Install-DockerDesktop {
    ...
    if (-not $installed) {
        ...
        try {
            Download-AetherSearchFile -Url "https://desktop.docker.com/win/main/amd64/Docker%20Desktop%20Installer.exe" -Output $installerPath
            ...
            if ($proc.ExitCode -eq 0) {
                ...
                if (-not $wslReady) { Print-AetherSearchError "WSL2 is required. Run: wsl --install --no-distribution, then reboot." }
    ...
    if (-not $installed) {
        Print-AetherSearchError "Could not install Docker Desktop automatically."
    ...
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
        Print-AetherSearchError "Docker installed but 'docker' command not available. Restart your terminal and re-run."
        exit 1
    }
    if (-not (Start-DockerDaemon)) {
        Print-AetherSearchError "Docker Desktop installed but could not be started. Launch it from the Start Menu and re-run."
        exit 1
    }
...
function Invoke-WslInstall {
    ...
    Print-Info "AetherSearch will be installed via WSL2 (Windows Subsystem for Linux)."
    if (-not (Confirm-Action "AetherSearch via WSL2 (installs WSL2 + Ubuntu + Docker inside Linux)")) { exit 1 }
    if (-not (Test-IsAdmin)) { Invoke-ElevatedRelaunch }
    ...
    try {
        ...
        if ($totalGB -lt 4) {
            Print-AetherSearchError "AetherSearch requires at least 4GB RAM (Lite mode) or 10GB RAM (Standard mode)."
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        try {
            ...
            if ($proc.ExitCode -ne 0) {
                Print-AetherSearchError "WSL2 installation failed (code $($proc.ExitCode)). A reboot may be needed."
                Print-Info "After rebooting, re-run this script."
                exit 1
            }
        } catch {
            Print-AetherSearchError "Could not install WSL2: $_"
            exit 1
        }
    }
    ...
    if ($distros -notmatch "Ubuntu") {
        ...
        if ($proc.ExitCode -ne 0) {
            Print-AetherSearchError "Ubuntu installation failed. Try manually: wsl --install -d Ubuntu"
    ...
    if ($wslExit -eq 0) {
        Print-Success "AetherSearch installation complete (via WSL2)"
        # Determine the port AetherSearch is running on inside WSL
        Print-Info "AetherSearch should be accessible at http://localhost:3000"
        Print-Info "WSL2 automatically forwards ports to the Windows host."
    } else {
        Print-AetherSearchError "Installation inside WSL2 exited with code $wslExit"
...
function Main {
    if ($Help) { Show-AetherSearchHelp; return }
    if ($PSVersionTable.PSVersion.Major -lt 5) { Print-AetherSearchError "PowerShell 5+ required (found $($PSVersionTable.PSVersion))"; exit 1 }
    if ($script:LiteMode -and $script:IncludeCraftMode) {
        Print-AetherSearchError "-Lite and -IncludeCraft cannot be used together."
        exit 1
    }
    if ($script:LiteMode) { $script:ExpectedDockerRamGB = 4; $script:ExpectedDiskGB = 16 }
    if ($Shutdown)   { Invoke-AetherSearchShutdown; return }
    if ($DeleteData) { Invoke-AetherSearchDeleteData; return }
    ...
    Write-Host "Welcome to AetherSearch Installation Script (Windows)"
    Write-Host "=============================================="
    Print-Success "$edition detected"
    Write-Host "This script will:" -ForegroundColor Yellow
    Write-Host "1. Download deployment files for AetherSearch into a new '$($script:InstallRoot)' directory"
    ...
    if (-not (Initialize-ComposeCommand)) {
        if ($script:IsWindowsServer) {
            Install-ComposePlugin
            if (-not (Initialize-ComposeCommand)) { Print-AetherSearchError "Docker Compose could not be installed."; exit 1 }
        } else {
            Print-AetherSearchError "Docker Compose is not installed. Docker Desktop includes it."
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        if (-not (Start-DockerDaemon)) { Print-AetherSearchError "Could not start Docker. Start it manually and re-run."; exit 1 }
    }
    Print-Success "Docker daemon is running"
    if ($script:IsWindowsServer) { Fix-DockerCredStore }

    # Verify Docker is running Linux containers (AetherSearch images are Linux-based)
    $osType = ((Invoke-NativeQuiet -PassThru { docker info --format '{{.OSType}}' }) -join "").Trim()
    if ($osType -eq "windows") {
        Print-Warning "Docker is running in Windows containers mode, but AetherSearch requires Linux containers."
    ...
    if ($resourceWarning) {
        Print-Warning "AetherSearch recommends at least $($script:ExpectedDockerRamGB)GB RAM and $($script:ExpectedDiskGB)GB disk for standard mode."
    ...
    else { Print-Step "Downloading AetherSearch configuration files" }

    $composeDest = Join-Path $deploymentDir "docker-compose.yml"
    if (-not (Ensure-AetherSearchFile $composeDest "$($script:GitHubRawUrl)/docker-compose.yml" "docker-compose.yml")) { exit 1 }
    ...
    if (-not (Ensure-AetherSearchFile $envTemplateDest "$($script:GitHubRawUrl)/env.template" "env.template")) { exit 1 }
    if (-not (Ensure-AetherSearchFile (Join-Path $script:InstallRoot "data\nginx\app.conf.template") "$($script:NginxBaseUrl)/app.conf.template" "nginx/app.conf.template")) { exit 1 }
    if (-not (Ensure-AetherSearchFile (Join-Path $script:InstallRoot "data\nginx\run-nginx.sh") "$($script:NginxBaseUrl)/run-nginx.sh" "nginx/run-nginx.sh")) { exit 1 }
    if (-not (Ensure-AetherSearchFile (Join-Path $script:InstallRoot "README.md") "$($script:GitHubRawUrl)/README.md" "README.md")) { exit 1 }
    ...
    if ((Test-Path $composeDest) -and (Initialize-ComposeCommand)) {
        ...
        if ($running.Count -gt 0) {
            Print-AetherSearchError "AetherSearch services are currently running!"
    ...
    if (Test-Path $envFile) {
        ...
        Prompt-DeploymentMode -LiteOverlayPath $liteOverlayPath
        if ($script:LiteMode -and $script:IncludeCraftMode) {
            Print-AetherSearchError "-IncludeCraft cannot be used with Lite mode."
            exit 1
        }
        if ($script:LiteMode) { $script:ExpectedDockerRamGB = 4; $script:ExpectedDiskGB = 16 }
        ...
        if ($script:IncludeCraftMode -or $version -match '^craft-') {
            Set-EnvFileValue -Path $envFile -Key "ENABLE_CRAFT" -Value "true" -Uncomment
            Print-Success "AetherSearch Craft enabled"
        } else {
            Print-Info "AetherSearch Craft disabled (use -IncludeCraft to enable)"
        }
    ...
    if (-not $useLatest -and -not $Local) {
        ...
        try {
            ...
            try { Download-AetherSearchFile "$pinnedBase/data/nginx/app.conf.template" (Join-Path $script:InstallRoot "data\nginx\app.conf.template") } catch {}
            try { Download-AetherSearchFile "$pinnedBase/data/nginx/run-nginx.sh" (Join-Path $script:InstallRoot "data\nginx\run-nginx.sh") } catch {}
            if ($script:LiteMode) {
                try { Download-AetherSearchFile "$pinnedBase/docker_compose/$($script:LiteComposeFile)" $liteOverlayPath } catch {}
            }
    ...
    if ((Invoke-Compose @pullArgs) -ne 0) { Print-AetherSearchError "Failed to download Docker images"; exit 1 }
    Print-Success "Docker images downloaded successfully"

    # ── Step 8: Start Services ────────────────────────────────────────────
    Print-Step "Starting AetherSearch services"
    Print-Info "Launching containers..."
    $upArgs = @("up", "-d")
    if ($useLatest) { $upArgs += @("--pull", "always", "--force-recreate") }
    $upResult = Invoke-Compose @upArgs
    if ($upResult -ne 0) { Print-AetherSearchError "Failed to start AetherSearch services"; exit 1 }
    ...
    foreach ($cid in $containerIds) {
        ...
        if ($status -eq "running" -and $restarts -gt 2) {
            Print-AetherSearchError "$name is in a restart loop (restarted $restarts times)"; $restartIssues = $true
        } elseif ($status -eq "running") { Print-Success "$name is healthy" }
        elseif ($status -eq "restarting") { Print-AetherSearchError "$name is stuck restarting"; $restartIssues = $true }
        else { Print-Warning "$name status: $status" }
    }

    if ($restartIssues) {
        Print-AetherSearchError "Some containers are experiencing issues!"
        ...
        Print-Info "For help, contact: support@aethersearch.app"
        exit 1
    }
    ...
    if (Test-AetherSearchHealth -Port $port) {
        Write-Host "============================================" -ForegroundColor Green
        Write-Host "   AetherSearch service is ready!                   " -ForegroundColor Green
        Write-Host "============================================" -ForegroundColor Green
    } else {
        ...
        Write-Host "   AetherSearch containers are running              " -ForegroundColor Yellow
        Write-Host "============================================" -ForegroundColor Yellow
    }

    Print-Info "Access AetherSearch at: http://localhost:$port"
    ...
    Print-Info "For help or issues, contact: support@aethersearch.app"
}
