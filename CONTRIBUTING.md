# Contributing to AetherSearch

Hey there! We are so excited that you're interested in AetherSearch.
...
If you do not get a response within a week, feel free to email support@aethersearch.app and include the issue in the message.
...
The AetherSearch team will either provide a design doc and PRD for the feature or request one from you, the contributor. The scope and detail of the design will depend on the individual feature.

### 3. IP attribution for EE contributions

If you are contributing features to AetherSearch Enterprise Edition, you are required to sign the [IP Assignment Agreement](contributor_ip_assignment/EE_Contributor_IP_Assignment_Agreement.md).
...
- The AetherSearch team will pull resources and bandwidth from design, PM, and engineering to ensure that you have all the resources to build the feature to the quality required for merging.

Because this is a large investment from our team, we ask that you:

- Thoroughly read all the requirements of the design docs, engineering best practices, and try to minimize overhead for the AetherSearch team.
- Complete the feature in a timely manner to reduce context switching and an ongoing resource pull from the AetherSearch team.

---

## Development Setup

AetherSearch being a fully functional app, relies on some external software, specifically:
...
> **Note:**
> This guide provides instructions to build and run AetherSearch locally from source with Docker containers providing the above external software.
...
We also use `ty` for static type checking. AetherSearch is fully type-annotated, and we want to keep it that way! To run the ty checks manually:
...
3. From the dropdown at the top, select "Run All AetherSearch Services" and press the green play button
...
Now, visit http://localhost:3000 in your browser. You should see the AetherSearch onboarding wizard where you can connect your external LLM provider to AetherSearch.

You've successfully set up a local AetherSearch instance!
...
After Docker pulls and starts these containers, navigate to http://localhost:3000 to use AetherSearch.

If you want to make changes to AetherSearch and run those changes in Docker, you can also build a local version of the AetherSearch container images that incorporates your changes:
...
- Connector code (data to AetherSearch documents):
...
AetherSearch loosely follows the SemVer versioning standard.
A set of Docker containers will be pushed automatically to DockerHub with every tag.
You can see the containers [here](https://hub.docker.com/u/aethersearch).
...
If you are contributing features to AetherSearch Enterprise Edition (code under any `ee/` directory), you are required to sign the [IP Assignment Agreement](contributor_ip_assignment/EE_Contributor_IP_Assignment_Agreement.md) ([PDF version](contributor_ip_assignment/EE_Contributor_IP_Assignment_Agreement.pdf)).
