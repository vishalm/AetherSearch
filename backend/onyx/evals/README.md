# AetherSearch Evaluations

This directory contains the evaluation framework for testing and measuring the performance of AetherSearch's chat and retrieval systems.

## Overview

The evaluation system uses [Braintrust](https://www.braintrust.dev/) to run automated evaluations against test datasets. It measures the quality of responses generated by AetherSearch's chat system and can be used to track performance improvements over time.
...
```bash
aethersearch/backend$ python -m dotenv -f .vscode/.env run -- python aethersearch/evals/eval_cli.py --remote --api-key <SUPER_CLOUD_USER_API_KEY> --search-permissions-email <email account to reference> --remote --remote-dataset-name Simple
```

You can also run the CLI directly from the command line:

```bash
aethersearch$ python -m dotenv -f .vscode/.env run -- python backend/aethersearch/evals/eval_cli.py --local-dataset-path backend/aethersearch/evals/data/eval.json --search-permissions-email richard@aethersearch.app
```
Save the env var AETHERSEARCH_EVAL_API_KEY in your .env file so you don't have to specify it every time for triggering remote runs.
