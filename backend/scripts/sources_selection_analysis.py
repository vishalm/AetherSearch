...
from aethersearch.configs.app_configs import DOCUMENT_INDEX_NAME  # noqa: E402
from aethersearch.configs.constants import SOURCE_TYPE  # noqa: E402
...
class SelectionAnalysis:
    ...
    def do_request(self, query: str) -> dict:
        """Request the AetherSearch API

        Args:
            query (str): A query

        Returns:
            dict: The AetherSearch API response content
        """
        cookies = (
        ...
        try:
            ...
            if response.status_code != 200:
                color_output(
                    (
                        f"something goes wrong while requesting the AetherSearch API for the query '{query}': {response.text}"
                    ),
                    model="critical",
                )
                sys.exit(1)
            ...
            color_output(
                f"Unable to request the AetherSearch API for the query '{query}': {e}",
                model="critical",
            )
            sys.exit(1)
    ...
    def extract_content(self, contents: dict) -> dict:
        """Extract the content returns by the AetherSearch API

        Args:
            contents (dict): The aethersearch response content
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Currently, to get this script working when the AetherSearch Auth is "
    ...
    parser.add_argument(
        ...
        default=3000,
        help=(
            "The AetherSearch Web (not the API) port. We use the UI to forward the requests to the API. "
            "It should be '3000' for local dev and '80' if AetherSearch runs using docker compose."
        ),
    )
    parser.add_argument(
        "-q--queries",
