...
class TestGetStripePublishableKey:
    ...
    def setup_method(self) -> None:
        ...
        import ee.aethersearch.server.tenants.billing_api as billing_api

        billing_api._stripe_publishable_key_cache = None

    @pytest.mark.asyncio
    @patch("ee.aethersearch.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_OVERRIDE", None)
    @patch(
        "ee.aethersearch.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_URL",
    ...
    async def test_fetches_from_s3_when_no_override(self) -> None:
        ...
        from ee.aethersearch.server.tenants.billing_api import get_stripe_publishable_key
    ...
    @pytest.mark.asyncio
    @patch(
        "ee.aethersearch.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_OVERRIDE",
        "pk_test_override123",
    )
    ...
    async def test_rejects_invalid_env_var_key_format(self) -> None:
        ...
        assert exc_info.value.error_code is AetherSearchErrorCode.INTERNAL_ERROR
    ...
    @pytest.mark.asyncio
    @patch("ee.aethersearch.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_OVERRIDE", None)
    @patch("ee.aethersearch.server.tenants.billing_api.STRIPE_PUBLISHABLE_KEY_URL", None)
