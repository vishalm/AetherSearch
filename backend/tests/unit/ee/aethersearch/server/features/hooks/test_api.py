"""Unit tests for ee.aethersearch.server.features.hooks.api helpers.
...
- _raise_for_validation_failure: HookValidateStatus → AetherSearchError mapping
"""
...
from ee.aethersearch.server.features.hooks.api import _check_ssrf_safety
from ee.aethersearch.server.features.hooks.api import _raise_for_validation_failure
from ee.aethersearch.server.features.hooks.api import _validate_endpoint
...
class TestCheckSsrfSafety:
    ...
    def test_https_is_allowed(self) -> None:
        with patch("aethersearch.utils.url.socket.getaddrinfo") as mock_dns:
    ...
    def test_non_https_scheme_rejected(self, url: str) -> None:
        with pytest.raises(AetherSearchError) as exc_info:
            self._call(url)
        assert exc_info.value.error_code == AetherSearchErrorCode.BAD_GATEWAY
    ...
    def test_private_ip_is_blocked(self, ip: str) -> None:
        with (
            patch("aethersearch.utils.url.socket.getaddrinfo") as mock_dns,
            pytest.raises(AetherSearchError) as exc_info,
        ):
    ...
    def test_dns_resolution_failure_raises(self) -> None:
        ...
        with (
            patch(
                "aethersearch.utils.url.socket.getaddrinfo",
...
class TestValidateEndpoint:
    def _call(self, *, api_key: str | None = _API_KEY) -> HookValidateResponse:
        ...
        with patch("ee.aethersearch.server.features.hooks.api._check_ssrf_safety"):
    ...
    @patch("ee.aethersearch.server.features.hooks.api.httpx.Client")
...
class TestRaiseForValidationFailure:
    @pytest.mark.parametrize(
        ...
        [
            (HookValidateStatus.auth_failed, AetherSearchErrorCode.CREDENTIAL_INVALID),
            (HookValidateStatus.timeout, AetherSearchErrorCode.GATEWAY_TIMEOUT),
            (HookValidateStatus.cannot_connect, AetherSearchErrorCode.BAD_GATEWAY),
        ],
    )
    def test_raises_correct_error_code(
        self, status: HookValidateStatus, expected_code: AetherSearchErrorCode
    ) -> None:
