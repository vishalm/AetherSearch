...
@patch("aethersearch.configs.app_configs.DISABLE_VECTOR_DB", True)
...
@patch("aethersearch.configs.app_configs.DISABLE_VECTOR_DB", False)
@patch(
...
def test_file_reader_tool_available(vector_db_disabled: bool) -> None:
    ...
    # local reference after `from aethersearch.configs.app_configs import DISABLE_VECTOR_DB`.
    with patch(
        "aethersearch.tools.tool_implementations.file_reader.file_reader_tool.DISABLE_VECTOR_DB",
        vector_db_disabled,
    ):
