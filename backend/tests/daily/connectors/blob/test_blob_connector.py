...
from aethersearch.connectors.blob.connector import BlobStorageConnector
...
@pytest.mark.parametrize(
    "blob_connector", [(BlobType.S3, "aethersearch-connector-tests")], indirect=True
)
def test_blob_s3_connector(
    ...
    for doc in all_docs:
        ...
        if file_extension in AetherSearchFileExtensions.TEXT_AND_DOCUMENT_EXTENSIONS:
            assert len(section.text) > 0
        else:
            assert len(section.text) == 0
...
@pytest.mark.parametrize(
    "blob_connector",
    [(BlobType.R2, "aethersearch-daily-connector-tests", {"european_residency": True})],
    indirect=True,
)
...
@pytest.mark.parametrize(
    "blob_connector", [(BlobType.GOOGLE_CLOUD_STORAGE, "aethersearch-test-1")], indirect=True
)
