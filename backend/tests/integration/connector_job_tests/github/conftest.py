...
def _get_github_test_tokens() -> list[str]:
    ...
    Minimal setup:
    - Set AETHERSEARCH_GITHUB_PERMISSION_SYNC_TEST_ACCESS_TOKEN (token1)
    Optional:
    - Set AETHERSEARCH_GITHUB_PERMISSION_SYNC_TEST_ACCESS_TOKEN_CLASSIC (token2 / classic)

    If the classic token is provided, the GitHub suite will run twice (once per token).
    """
    token_1 = os.environ.get("AETHERSEARCH_GITHUB_PERMISSION_SYNC_TEST_ACCESS_TOKEN")
    # Prefer the new "classic" name, but keep backward compatibility.
    token_2 = os.environ.get("AETHERSEARCH_GITHUB_PERMISSION_SYNC_TEST_ACCESS_TOKEN_CLASSIC")
...
def github_access_token(request: pytest.FixtureRequest) -> str:
    ...
    if not tokens:
        pytest.skip(
            ...
            "AETHERSEARCH_GITHUB_PERMISSION_SYNC_TEST_ACCESS_TOKEN and "
            "AETHERSEARCH_GITHUB_PERMISSION_SYNC_TEST_ACCESS_TOKEN_CLASSIC"
        )
...
def github_test_env_setup(
    ...
    admin_email = os.environ.get("AETHERSEARCH_GITHUB_ADMIN_EMAIL", "admin@example.com")
    test_user_1_email = os.environ.get(
        "AETHERSEARCH_GITHUB_TEST_USER_1_EMAIL", "subash@aethersearch.app"
    )
    test_user_2_email = os.environ.get(
        "AETHERSEARCH_GITHUB_TEST_USER_2_EMAIL", "msubash203@gmail.com"
    )
