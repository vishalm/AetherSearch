...
- You should assume that all AetherSearch services are running. To verify, you can check the `backend/log` directory to
  make sure we see logs coming out from the relevant service.
- To connect to the Postgres database, use: `docker exec -it aethersearch-relational_db-1 psql -U postgres -c "<SQL>"`
- When making calls to the backend, always go through the frontend. E.g. make a call to `http://localhost:3000/api/persona` not `http://localhost:8080/api/persona`
- Put ALL db operations under the `backend/aethersearch/db` / `backend/ee/aethersearch/db` directories. Don't run queries
  outside of those directories.

## Project Overview

**AetherSearch** (formerly Danswer) is an open-source Gen-AI and Enterprise Search platform that connects to company documents, apps, and people. It features a modular architecture with both Community Edition (MIT licensed) and Enterprise Edition offerings.

### Background Workers (Celery)

AetherSearch uses Celery for asynchronous task processing with multiple specialized workers:
...
There are 4 main types of tests within AetherSearch:

### Unit Tests

These should not assume any AetherSearch/external services are available to be called.
...
These tests assume that all external dependencies of AetherSearch are available and callable (e.g. Postgres, Redis,
MinIO/S3, Vespa are running + OpenAI can be called + any request to the internet is fine + etc.).

However, the actual AetherSearch containers are not running and with these tests we call the function to test directly.
...
Standard integration tests. Every test in `backend/tests/integration` runs against a real AetherSearch deployment. We cannot
...
These tests are an even more complete version of the Integration Tests mentioned above. Has all services of AetherSearch
...
to logs via the `backend/log/<service_name>_debug.log` file. All AetherSearch services (api_server, web_server, celery_X)
...
**Always raise `AetherSearchError` from `aethersearch.error_handling.exceptions` instead of `HTTPException`.
Never hardcode status codes or use `starlette.status` / `fastapi.status` constants directly.**

A global FastAPI exception handler converts `AetherSearchError` into a JSON response with the standard
...
raise AetherSearchError(AetherSearchErrorCode.BAD_GATEWAY, detail, status_code_override=upstream_status)
...
Available error codes are defined in `backend/aethersearch/error_handling/error_codes.py`. If a new error
...
```python
raise AetherSearchError(AetherSearchErrorCode.BAD_GATEWAY, detail, status_code_override=e.response.status_code)
```
